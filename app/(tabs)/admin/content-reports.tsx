...
import type { ContentReport } from '@/data/admin/adminApi';
...
function ReportCard({ item, onResolve, onDismiss, busy }: CardProps) {
  const typeLabel = item.targetType === 'thread' ? 'Thread' : 'Reply';
  ...
  return (
    <View style={styles.card}>
      ...
      <Text style={styles.cardLabel}>
        {typeLabel} reported by {item.reporterName}
      </Text>
...
export default function ContentReportsScreen() {
  ...
  const handleResolve = (reportId: string) => {
    Alert.alert(
      'Remove content?',
      'This will mark the reported content for removal.',
      [
  ...
  const renderItem = ({ item }: { item: ContentReport }) => (
    <ReportCard
      item={item}
